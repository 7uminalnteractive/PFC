...
CREATE POLICY "games_insert" ON public.games
  ...
  WITH CHECK (
    (SELECT email FROM public.profiles WHERE id = auth.uid()) = 'admin@bolao.com'
  );
...
--
-- IMPORTANTE: Se você já tinha as policies antigas rodando,
-- execute antes:
--   DROP POLICY IF EXISTS "games_insert" ON public.games;
--   DROP POLICY IF EXISTS "games_update" ON public.games;
--   DROP POLICY IF EXISTS "games_delete" ON public.games;
--   DROP POLICY IF EXISTS "guesses_select_admin" ON public.guesses;
